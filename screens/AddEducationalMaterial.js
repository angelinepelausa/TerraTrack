...
const AddEducationalMaterial = () => {
  ...
  // pick image only (upload handled on save)
  const pickImage = () => {
  ...
  const handleAddQuestion = () =>
    setQuiz([...quiz, { text: '', options: ['', '', '', ''], correctIndex: 0 }]);
  ...
  const handleQuestionChange = (qIndex, field, value) => {
    ...
    if (field === 'text') updated[qIndex].text = value;
      ...
      updated[qIndex].correctIndex = parseInt(value || '0', 10);
    }
    setQuiz(updated);
  };
      ...
      <View style={styles.searchRow}>
        ...
        >
          <View style={styles.filterCircle}>
            <Text style={{ color: '#131313', fontWeight: '600' }}>Add</Text>
          </View>
        </TouchableOpacity>
      </View>
